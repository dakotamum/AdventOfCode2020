...
history = {}
for i in range(len(numbersAlreadyMentioned) - 1):
    number = numbersAlreadyMentioned[i]
    history[number] = i + 1
currentIteration = len(numbersAlreadyMentioned)
currentNumberOfInterest = numbersAlreadyMentioned[len(numbersAlreadyMentioned) - 1]

while currentIteration < 30000000:
    if currentNumberOfInterest not in history:
        history[currentNumberOfInterest] = currentIteration
        currentNumberOfInterest = 0
    else:
        newNumber = currentIteration - history[currentNumberOfInterest]
        history[currentNumberOfInterest] = currentIteration
        currentNumberOfInterest = newNumber 
    currentIteration += 1
...
while currentIteration <= 2020:
    ...
    for i in reversed(range(len(numbersAlreadyMentioned) - 1)):
        ...
        if numbersAlreadyMentioned[i] == lastNumber:
            ...
            break if foundAnotherLastNumber == False:
