...
indexOfBadNumber = preamble
arrayThatMightAddToBadNumber = []
...
#create list of numbers that could add up to the bad number
for badIter in range(0, indexOfBadNumber):
    arrayThatMightAddToBadNumber.append(numbers[badIter])

lowBadNumber = 0
highBadNumber = 0

currentJArray = arrayThatMightAddToBadNumber[:]

for iteri, val in enumerate(arrayThatMightAddToBadNumber):
    itAllAddsUp = 'false'
    currentTotal = val
    lowestNumber = val
    highestNumber = val
    currentJArray.pop(0)
    #print("iteri val: " + str(val))
    #print("current J array: ")
    #print(currentJArray)
    for iterj, value in enumerate(currentJArray, start=1):
        #print("current total: " + str(currentTotal))
        #print("iterj val: " + str(value))
        if currentTotal + value == numberThatDoesntAddUp:
            itAllAddsUp = 'true'
            print("found it!")
            lowBadNumber = lowestNumber
            highBadNumber = highestNumber
            if value > highestNumber:
                highestNumber = value
            if value < lowestNumber:
                lowestNumber = value
            break
        elif currentTotal + value > numberThatDoesntAddUp:
            break
        elif value > highestNumber:
            highestNumber = value
        elif value < lowestNumber:
            lowestNumber = value
        currentTotal += value
    if itAllAddsUp == 'true':
        break
